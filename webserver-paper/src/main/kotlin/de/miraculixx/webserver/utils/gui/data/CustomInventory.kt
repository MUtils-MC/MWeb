package de.miraculixx.webserver.utils.gui.data
...
import de.miraculixx.webserver.utils.messages.emptyComponent
...
abstract class CustomInventory(
    ...
    fun open(player: Player) {
        if (settings.debug) consoleAudience.sendMessage(prefix + cmp("Open GUI '$id' to ${player.name}"))
        viewers.add(player)
    ...
    private val onClose = listen<InventoryCloseEvent> {
        ...
        if (viewers.isEmpty()) {
            if (settings.debug) consoleAudience.sendMessage(prefix + cmp("GUI removed: $id"))
            InventoryManager.remove(id)
        }
    }
