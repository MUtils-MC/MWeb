...
package de.miraculixx.webserver.utils.serializer
...
class Config(stream: InputStream?, val name: String, private val destination: File) {
    ...
    inline fun <reified T> get(key: String): T? {
        ...
        key.split('.').forEach {
            if (route is Map<*, *>) {
                ...
                if (settings.debug) consoleAudience.sendMessage(prefix + cmp("Failed to resolve config key '$key' because '$it' is no section or null ('$name.yml')"))
                return null
            }
        }
        return if (route is T) route as T else {
            if (settings.debug) consoleAudience.sendMessage(prefix + cmp("Failed to cast config value for key '$key' in config '$name.yml'"))
            null
        }
    }
    ...
    private fun loadConfig(file: File, input: InputStream) {
        if (settings.debug) consoleAudience.sendMessage(prefix + cmp("Create new config file - $name"))
        if (!file.exists()) {
            file.createNewFile()
    ...
    init {
        ...
        if (settings.debug) consoleAudience.sendMessage(prefix + cmp("Load config file - $name"))
