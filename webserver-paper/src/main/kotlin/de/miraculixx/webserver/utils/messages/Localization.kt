package de.miraculixx.webserver.utils.messages

import de.miraculixx.webserver.utils.consoleAudience
import de.miraculixx.webserver.utils.miniMessage
...
class Localization(private val folder: File, active: String, keys: List<Pair<String, InputStream?>>) {
    ...
    fun setLanguage(key: String): Boolean {
        ...
        if (config.get<String>("version") == null) {
            if (settings.debug) consoleAudience.sendMessage(prefix + cmp("LANG - $key file is not a valid language config"))
            return false
        }
    ...
    private fun checkFiles() {
        ...
        folder.listFiles()?.forEach {
            if (settings.debug) consoleAudience.sendMessage(prefix + cmp("LANG - Founded language file '${it.name}'! Load it"))
